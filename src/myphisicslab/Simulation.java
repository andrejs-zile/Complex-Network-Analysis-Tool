 ...
 */
/*
  //MODIFICATIONS BY ANDREJS ZILE:
  Couple of modifications were made to this file to make it work
  for the simulation of the network.
  - added variable for time multiplier
  - added variable for timeNowReset - which resets the now time
  - added variable for simulationActive
  - added variable for realTime - which basically represents the

  Main modifications done to methods:
  - getTime() - changed values and if statement
  - getTimeStep() - changed values
  - run() - added if statement
...
public abstract class Simulation implements Runnable, Graphable, MouseDragHandler,
  ...
  public double stepperAltTime = 0;
  protected double timeMultiplier = 8;
  protected boolean timeNowReset = false;
  ...
  protected Container getCurrentContainer() { return this.container; }
  ...
  public double getTime() {
    ...
    if (realTime) {
      now = systemTime/(1000.0);//8.0);
    } else {
      now = systemTime/(1000.0/timeMultiplier);
    }
    if (startTime == 0 || now < startTime || timeNowReset) {
      // initialize startTime... used for debugging
      startTime = now;
      //stopMotion();
      timeNowReset = false;
    }
  ...
  public double getTimeStep() {
    ...
    */
    ...
    //System.out.println("Step count: " + stepCount++  + ", Time step of: " + h + " and get time is equal to: " + getTime());
    lastTime = now;
    h = 0.01 * timeMultiplier;
    if (simulationActive) {
      return h;
    } else {
      return 0;
